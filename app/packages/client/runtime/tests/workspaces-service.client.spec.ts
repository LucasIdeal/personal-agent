...
describe('WorkspaceRuntime', () => {
  ...
  it('targets New Session at an explicit Workspace and clears into the home chooser otherwise', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    workspaces.startSession(wid('current-home'))
    await Promise.resolve()
...
describe('startInitialSelection', () => {
  ...
  it('does not auto-connect a Workspace when no session is current', async () => {
    const b = bench()
  ...
  it('stays idle when a session is already current or no recent Workspace exists', async () => {
    ...
    expect(withCurrent.sessions.list.getSnapshot().current).toBe('s1')
    stopCurrent()
  ...
  it('clears a restored blank current so the home chooser can show', async () => {
    const b = bench()
    b.api.onList = () => Promise.resolve(ok({
      items: [{ sessionId: sid('s-blank'), updatedAt: 1, running: false, blank: true }] as never[],
    }))
    await b.sessions.refresh()
    b.sessions.open(sid('s-blank'))
    expect(b.sessions.list.getSnapshot().current).toBe('s-blank')
    const stop = b.workspaces.startInitialSelection()
    b.api.onWorkspaceList = () => Promise.resolve(ok({ items: [workspace('w1')] as never[] }))
    await b.workspaces.refresh()
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(b.sessions.list.getSnapshot().current).toBeUndefined()
    expect(b.api.callsOf('session.create')).toHaveLength(0)
  ...
  it('list changes after baselines land still do not auto-connect', async () => {
    const b = bench()
    b.api.onWorkspaceList = () => Promise.resolve(ok({
      items: [workspace('recent', [], '2026-01-02T00:00:00.000Z')] as never[],
    }))
