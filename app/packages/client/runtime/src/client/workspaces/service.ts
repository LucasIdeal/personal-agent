...
export class WorkspaceRuntime implements IWorkspaces {
  ...
  /**
   * Wait for the first complete Workspace/Session baseline, then stop. A
   * restored current session with history is left in place; a blank current
   * (including the previous auto-selected chat-home draft) is cleared so the
   * home chooser can show. Connecting a Workspace or chat-home is an explicit
   * user action, not a startup policy. Later explicit clears stay cleared
   * instead of retriggering this one-shot.
  ...
  startInitialSelection(): () => void {
    ...
    const reconcile = (): void => {
      ...
      if (!this.list.getSnapshot().baselinesReady) return
      done = true
      const sessions = this.sessions.list.getSnapshot()
      const current = sessions.current
      if (current === undefined) return
      if (sessions.byId[current]?.blank === true) this.sessions.clear()
    }
   ...
   * button, workspace browser). An explicit Workspace still connects that
   * workspace's blank session. With no target, clear into the home chooser
   * (workspace vs chat) instead of assuming the current or recent Workspace.
