...
describe('WorkspacePicker', () => {
  it('lists chat-home as Chat above project folders', () => {
    const b = mount([
      { ...workspace('alpha', 'Alpha'), path: '/projects/alpha' },
      {
        workspaceId: wid('chat'), path: '/Users/me/.dsh/chat', title: '对话',
        sessionIds: [], createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z',
      },
    ])
    const items = screen.getAllByRole('menuitem').map(item => item.textContent)
    expect(items[0]).toBe('纯聊天')
    expect(items).toContain('Alpha')
    expect(items).not.toContain('对话')
    fireEvent.click(screen.getByRole('menuitem', { name: '纯聊天' }))
    expect(b.onPick).toHaveBeenCalledWith(wid('chat'))
  })
