...
function mount(overrides: Partial<WorkspaceBrowserProps> = {}) {
  ...
  store.actions.setListMode('workspace')
...
describe('WorkspaceBrowser', () => {
  ...
  it('filters the list between chat-home and project workspaces', () => {
    const sessions = sessionState([summary('alpha-s', 2), summary('chat-s', 1)])
    const b = mount({
      useSessions: hook(sessions),
      useWorkspaces: hook(workspaceState([
        workspace('alpha', ['alpha-s']),
        {
          workspaceId: wid('chat'), path: '/tmp/.dsh/chat', title: '对话',
          sessionIds: [sid('chat-s')], createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z',
        },
      ])),
    })
    expect(screen.getByRole('button', { name: '工作区' }).getAttribute('aria-pressed')).toBe('true')
    expect(screen.getAllByText('纯聊天')).toHaveLength(1)
    expect(screen.getAllByText('工作区')).toHaveLength(1)
    expect(screen.getByText('alpha')).toBeTruthy()
    expect(screen.queryByText('对话')).toBeNull()

    fireEvent.click(screen.getByRole('button', { name: '纯聊天' }))
    expect(b.store.getSnapshot().listMode).toBe('chat')
    expect(screen.getByRole('button', { name: '纯聊天' }).getAttribute('aria-pressed')).toBe('true')
    expect(screen.queryByText('alpha')).toBeNull()
    expect(screen.getByText('对话')).toBeTruthy()
    expect(screen.getByText('chat-s')).toBeTruthy()
    expect(screen.queryByLabelText('添加工作区')).toBeNull()

    fireEvent.click(screen.getByRole('button', { name: '工作区' }))
    expect(b.store.getSnapshot().listMode).toBe('workspace')
    expect(screen.getByText('alpha')).toBeTruthy()
    expect(screen.queryByText('对话')).toBeNull()
    expect(screen.getByLabelText('添加工作区')).toBeTruthy()
  })

  it('keeps programmatic flat grouping for the session list', () => {
