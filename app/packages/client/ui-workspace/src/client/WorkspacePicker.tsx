...
import {
  Button, FishLogo, IconFolderClose16, IconPlusOutline16, Menu, Modal, type MenuEntry,
...
export function WorkspacePickFlow({
  ...
  const chatHome = workspaces.find(workspace => isChatHomePath(workspace.path))
  const projectWorkspaces = workspaces.filter(workspace => !isChatHomePath(workspace.path))
  ...
  const addEntries: MenuEntry[] = flowAvailable
    ...
    : []
  const chatEntry: MenuEntry | undefined = chatHome === undefined
    ? undefined
    : { id: chatHome.workspaceId, label: t('mode.chat'), icon: <FishLogo size={16} />, disabled: flowBusy }
  const folderEntries: MenuEntry[] = projectWorkspaces.map(workspace => ({
    id: workspace.workspaceId,
    label: workspace.title,
    icon: <IconFolderClose16 size={16} />,
    disabled: flowBusy,
  }))
  const listed: MenuEntry[] = [
    ...(chatEntry === undefined ? [] : [chatEntry]),
    ...(chatEntry !== undefined && folderEntries.length > 0
      ? [{ type: 'separator' as const, id: 'workspace-folders' }]
      : []),
    ...folderEntries,
  ]
  // With workspaces listed, the add action pins below the scroll region
  // (divider + always visible); otherwise it IS the menu.
  const pinAdd = !addOnly && listed.length > 0
  const items: MenuEntry[] = pinAdd ? listed : addEntries
