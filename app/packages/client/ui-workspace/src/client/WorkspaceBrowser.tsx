 ...
 * `sidebar.workspaces` hole: section header (chat/workspace switch + add
...
import {
  Button, FishLogo, IconCloseFill14, IconFolderClose16,
...
import { isChatHomePath } from './chat-home.ts'
...
/** Sidebar list filter: chat-home sessions or project workspaces. */
function ListModeSwitch({ listMode, onPick, t, className }: {
  listMode: 'chat' | 'workspace'
  onPick: (mode: 'chat' | 'workspace') => void
  t: WorkspaceBrowserProps['t']
  className?: string
}) {
  return (
    <div className={clsx(css.modeSwitch, className)} role="group" aria-label={t('section.sessions')}>
      <button
        type="button"
        className={css.modeButton}
        aria-pressed={listMode === 'chat'}
        onClick={() => { onPick('chat') }}
      >
        <FishLogo size={14} />
        {t('mode.chat')}
...
> & {
  ...
  /**
   * Every registered Workspace, used to decide Ungrouped membership so a
   * list-mode filter cannot spill the other mode's sessions into Ungrouped.
   */
  accountWorkspaces: readonly WorkspaceView[]
  /** Hide the Ungrouped bucket (chat-home list has no leftover sessions). */
...
function SessionTree({
  useSessions, startSession, open, forkSession, workspaces, accountWorkspaces, hideUngrouped, archivedSessionIds,
  ...
  const ungroupedSessionIds = useMemo(() => {
    if (hideUngrouped) return []
    const accounted = new Set(accountWorkspaces.flatMap(workspace => workspace.sessionIds))
    return list.ids.filter(id => list.byId[id] !== undefined && !accounted.has(id))
  }, [accountWorkspaces, hideUngrouped, list])
  useEffect(() => {
...
function SearchResults({
  ...
  resultLimit: number
  includeSession: (sessionId: SessionId) => boolean
}) {
  const list = useSessions(s => s)
  ...
  const results = useMemo(() => {
    const derived = deriveSearchResults(list, workspaces, query, archivedSessionIds, currentRemote, resultLimit)
    return {
      ...derived,
      items: derived.items.filter(item => includeSession(item.id)),
    }
  }, [list, workspaces, query, archivedSessionIds, currentRemote, resultLimit, includeSession])
...
export function WorkspaceBrowser({
  ...
  const chatWorkspaces = useMemo(
    () => workspaces.filter(workspace => isChatHomePath(workspace.path)),
    [workspaces],
  )
  ...
  const visibleWorkspaces = listMode === 'chat' ? chatWorkspaces : projectWorkspaces
  const chatSessionIds = useMemo(
    () => new Set(chatWorkspaces.flatMap(workspace => workspace.sessionIds)),
    [chatWorkspaces],
  )
  useEffect(() => {
    if (listMode !== 'chat') return
    for (const workspace of chatWorkspaces) {
      const key = workspace.workspaceId as string
      if (!Object.hasOwn(groupExpansion, key)) actions.setGroupExpanded(key, true)
    }
  }, [actions, chatWorkspaces, groupExpansion, listMode])
  useEffect(() => {
    ...
    <div className={clsx(css.root, !wide && css.rail)}>
      <div className={css.sectionHeader}>
        {wide && (
          <ListModeSwitch
            listMode={listMode}
            onPick={(mode) => { actions.setListMode(mode) }}
            t={t}
            className={clsx(css.wide, searchExpanded && css.modeSwitchHidden)}
          />
        )}
        {wide && (
        ...
        <div className={clsx(css.headerActions, wide && searchExpanded && css.headerActionsHidden)}>
          {/* Adding is the button's one action, so a composition with no
              ...
              button rather than leaving a dead one in the header. Chat mode
              has no project folder to add. */}
          {directoryFlowAvailable && listMode === 'workspace' && (
          ...
          ? (
            <SearchResults
              ...
              includeSession={sessionId =>
                listMode === 'chat' ? chatSessionIds.has(sessionId) : !chatSessionIds.has(sessionId)}
              t={t}
            />
          )
          : groupBy === 'flat'
