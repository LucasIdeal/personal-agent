...
describe('conversation slot inject API', () => {
  ...
  it('startChat opens the built-in chat-home workspace', async () => {
    ...
    b.runtime.workspaces.stub('connectWorkspace', () => Promise.resolve(ROOT))
    await resident.startChat()
    expect(b.runtime.workspaces.calls).toContainEqual({ method: 'connectWorkspace', args: ['chat-home'] })
    expect(b.runtime.sessions.calls).toContainEqual({ method: 'open', args: [ROOT] })
    await b.runtime.dispose()
  })

  it('startChat rejects when chat-home is not registered', async () => {
    const b = await bench()
    await expect(b.residentApi(undefined).startChat()).rejects.toThrow(/chat-home/)
    await b.runtime.dispose()
  })
