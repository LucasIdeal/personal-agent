...
function mount(
  ...
  options: {
    ...
    viewTabs?: ViewTab[]
    /** Render the resident shell with no current session (home chooser). */
    noSession?: boolean
    /** Session cwd override (chat-home path vs a project folder). */
    cwd?: string
    /** Homepage "纯聊天" action. */
    startChat?: () => Promise<void>
  } = {},
) {
  const startChat = options.startChat ?? vi.fn(async () => {})
  const root = sid('root')
  const rootRow = { id: root, displayTitle: 'Root', running: false, blank: false, updatedAt: 1 }
  const childRow = {
    id: SID, displayTitle: 'Child', parentId: root, cwd: options.cwd ?? '/projects/one',
...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    ...
    expect(view.getByText('Dept Review')).toBeTruthy()
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('lets the no-workspace posture win over a block', () => {
    ...
    fireEvent.click(b.view.getByRole('button', { name: /基于工作区/ }))
    const box = b.view.getByRole('textbox') as HTMLTextAreaElement
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('智能助理')).toBeTruthy()
    expect(b.view.getByText('部门答辩')).toBeTruthy()
  ...
  it('settling phase: a summary that does not prove the session blank hides the composer while it opens', () => {
    ...
    expect(b.view.queryByText('智能助理')).toBeNull()
  })
  ...
  it('home chooser offers workspace and chat; workspace reveals the picker, chat calls startChat', async () => {
    const startChat = vi.fn(async () => {})
    const b = mount(
      conversationSnapshot({ composerPhase: 'blank', blank: true }),
      [],
      vi.fn(async () => {}),
      { noSession: true, startChat },
    )
    expect(b.view.getByText('基于工作区')).toBeTruthy()
    ...
    expect(b.view.getByRole('button', { name: '返回' })).toBeTruthy()
    const box = b.view.getByRole('textbox') as HTMLTextAreaElement
    expect(box.readOnly).toBe(true)

    fireEvent.click(b.view.getByRole('button', { name: '返回' }))
    expect(b.view.getByText('基于工作区')).toBeTruthy()

    fireEvent.click(b.view.getByRole('button', { name: /纯聊天/ }))
    expect(startChat).toHaveBeenCalled()
  })

  it('keeps the chooser when startChat rejects', async () => {
    const startChat = vi.fn(async () => { throw new Error('missing chat-home') })
    const b = mount(
      conversationSnapshot({ composerPhase: 'blank', blank: true }),
      [],
      vi.fn(async () => {}),
  ...
  it('chat-home session shows a chat badge and a live composer', () => {
    const b = mount(
      conversationSnapshot({ composerPhase: 'blank', blank: true }),
      [{
        workspaceId: wid('chat'),
        path: '/Users/me/.dsh/chat',
        title: '对话',
        sessionIds: [SID],
      ...
      undefined,
      { cwd: '/Users/me/.dsh/chat' },
    )
    expect(b.view.getByRole('button', { name: '纯聊天' })).toBeTruthy()
    expect(b.view.queryByRole('button', { name: '选择工作区' })).toBeNull()
    const box = b.view.getByRole('textbox') as HTMLTextAreaElement
    expect(box.disabled).toBe(false)
    expect(box.readOnly).toBe(false)
    expect(box.placeholder).toBe('发消息给智能助理')
    fireEvent.click(b.view.getByRole('button', { name: '纯聊天' }))
    const owner = b.pickerOwner() as { open: boolean }
    expect(owner.open).toBe(true)
  })

  it('treats a chat-home cwd as chat even when the workspace list has not claimed the session', () => {
    const b = mount(
      conversationSnapshot({ composerPhase: 'blank', blank: true }),
      [],
      undefined,
      { cwd: '/tmp/.dsh/chat', summaryBlank: true },
    )
    expect(b.view.getByRole('button', { name: '纯聊天' })).toBeTruthy()
    const box = b.view.getByRole('textbox') as HTMLTextAreaElement
    expect(box.readOnly).toBe(false)
