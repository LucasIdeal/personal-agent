 ...
 * pass through. Known presets use the locale dictionary instead.
 */
...
function optionLabel(option: PermissionSelectValue['options'][number], t: ComposerBarProps['t']): string {
  if (option.value === 'read-only') return t('preset.read-only')
  if (option.value === 'workspace-write') return t('preset.workspace-write')
  if (option.value === FULL_ACCESS) return t('preset.full-access')
      ...
      <Menu
        ...
        anchor={
          <button
            ...
            aria-label={t('input.accessMode', { name: current === undefined ? optionLabel({ value: currentValue, name: currentValue }, t) : optionLabel(current, t) })}
          ...
          >
            ...
            <span className={css.triggerLabel}>{current === undefined ? optionLabel({ value: currentValue, name: currentValue }, t) : optionLabel(current, t)}</span>
