...
export function apply(ctx: Context): void {
  ...
  slots.register({
    ...
    inject: (sessionId: SessionId | undefined): ConversationInjected => {
      const openWorkspace = async (workspaceId: Parameters<ConversationInjected['selectWorkspace']>[0]) => {
      ...
      return {
        ...
        startChat: async () => {
          const chat = workspaces.list.getSnapshot().items.find(item => isChatHomePath(item.path))
          if (chat === undefined) {
            throw new Error('ui-conversation: chat-home workspace is not registered')
          }
          await openWorkspace(chat.workspaceId)
        },
      }
    },
  }, ConversationRoot)
